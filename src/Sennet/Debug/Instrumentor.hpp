...
public:
		...
		const std::string& filepath = "results.json")
	{
		...
		if (m_CurrentSession)
		{
			...
			{
				SN_CORE_ERROR("Instrumentor::BeginSession("
		...
		else
		{
			if (Log::GetCoreLogger())
			{
				SN_CORE_ERROR("Instrumentor could not open "
					"results file '{0}'", filepath);
			}
		}
	}
...
class InstrumentationUtils
{
	...
	#define SN_PROFILE_END_SESSION() \
		::Sennet::Instrumentor::Get().EndSession();
